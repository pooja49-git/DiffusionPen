...
class ResBlock(TimestepBlock):
    def __init__(self, channels, emb_channels, dropout, out_channels=None, use_conv=False, use_scale_shift_norm=False, dims=2, up=False, down=False):
        ...
        self.in_layers = nn.Sequential(Normalize(channels), nn.SiLU(), nn.Conv2d(channels, self.out_channels, 3, padding=1))
        self.updown = up or down
        if up:
            self.h_upd = Upsample(channels, False, dims)
            self.x_upd = Upsample(channels, False, dims)
        ...
        self.out_layers = nn.Sequential(Normalize(self.out_channels), nn.SiLU(), nn.Dropout(p=dropout), zero_module(nn.Conv2d(self.out_channels, self.out_channels, 3, padding=1)))
...
# ===================================================================
# ## Main UNetModel Class
# ===================================================================
class UNetModel(nn.Module):
    def __init__(self, image_size, in_channels, model_channels, out_channels, num_res_blocks, attention_resolutions, dropout=0, channel_mult=(1, 2, 4, 8), conv_resample=True, dims=2, num_classes=None, use_checkpoint=False, use_fp16=False, num_heads=-1, num_head_channels=-1, use_scale_shift_norm=False, resblock_updown=False, use_spatial_transformer=True, context_dim=768, vocab_size=None, args=None, text_encoder=None):
        super().__init__()
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                layers = [ResBlock(ch, time_embed_dim, dropout, out_channels=model_channels * mult, dims=dims, use_scale_shift_norm=use_scale_shift_norm)]
                ch = model_channels * mult
                if ds in attention_resolutions:
                    dim_head = ch // num_heads
                    layers.append(SpatialTransformer(ch, num_heads, dim_head, depth=1, context_dim=context_dim))
                self.input_blocks.append(TimestepEmbedSequential(*layers))
                input_block_chans.append(ch)
            if level != len(channel_mult) - 1:
                out_ch = ch
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            SpatialTransformer(ch, num_heads, dim_head, depth=1, context_dim=context_dim),
            ResBlock(ch, time_embed_dim, dropout, dims=dims, use_scale_shift_norm=use_scale_shift_norm)
        )
        ...
        self.interpolation = args.interpolation
        self.mix_rate = args.mix_rate
        # --- FIXED: Changed 320 back to 1280 to match the style encoder's output ---
        self.style_lin = nn.Linear(1280, time_embed_dim)

    def forward(self, x, timesteps=None, context=None, y=None, style_extractor=None, **kwargs):
        hs = []
        ...
        if context is not None:
            if self.text_encoder is None:
                raise ValueError("text_encoder must be provided to UNetModel for context processing.")
